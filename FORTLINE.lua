--[[ 
    FORTLINE PRO — FULL UPDATE & BUG FIXED
...
local Window = Rayfield:CreateWindow({
    Name = "FORTLINE PRO Updated",
    Icon = 0,
    LoadingTitle = "Rayfield Suite",
    LoadingSubtitle = "by Zero",
    Theme = "Default",
    ConfigurationSaving = {Enabled=true, FolderName=nil, FileName="FORTLINE PRO Updated"},
})
...
local function FireWeapon(target)
    if target and target.Parent then
        pcall(function()
            ...
            network:FireServer(LocalPlayer.Backpack.RocketLauncher, {
...
TabMain:CreateButton({Name="LoopKill All", Callback=LoopKillAll})
...
TabMain:CreateInput({Name="LoopKill Player", CurrentValue="", PlaceholderText="Name", RemoveTextAfterFocusLost=false, Callback=function(txt)
    task.spawn(function()
        while task.wait(0.5) do
            local player = Players:FindFirstChild(txt)
            if player and player.Character and player.Character:FindFirstChild("Head") then
...
TabMain:CreateButton({Name="God (One-Life)", Callback=GodOneLife})
TabMain:CreateButton({Name="God (Permanent)", Callback=GodPermanent})
...
-- =======================
local PlayersESP = {}
local ESPSettings = {PlayerESP=true, ShowNames=true, ShowHealth=true, ShowDistance=true, TeamColor=true}
...
RunService.RenderStepped:Connect(function()
    for _,player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            local box = PlayersESP[player].Box
            local text = PlayersESP[player].Text
...
local AimbotFOV = 100
local AimbotBone = "HumanoidRootPart"
...
RunService.RenderStepped:Connect(function()
    if AimbotEnabled then
        ...
        if target and target.Character and target.Character:FindFirstChild(AimbotBone) then
            Camera.CFrame = CFrame.new(Camera.CFrame.Position, target.Character[AimbotBone].Position)
        end
    end
end)
